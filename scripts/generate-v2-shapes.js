...
const output = path.join(root, 'resources/v2/shapes');
const modulePath = path.join(root, 'build/v2/src/v2/legacy/profiles.js');
...
if (check && changed) {
  console.error('v2 Shape resources are stale; run npm run generate:v2:shapes');
  process.exit(1);
}
