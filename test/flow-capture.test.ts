...
import { Application, type TaskRequest } from '../src/app/index.js';
...
test('flow capture reuses workers and Plans but isolates every Realm and request context', async (t) => {
  ...
  const originalPlan = Application.prototype.plan;
  const plans: Awaited<ReturnType<typeof originalPlan>>[] = [];
  t.mock.method(Application.prototype, 'plan', async function (this: Application, request: TaskRequest) {
