...
import { runCli, type CliIo, type CliServerHandle } from '../src/cli.js';

test('CLI collect starts the dedicated collector with explicit storage and closes cleanly', async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'mimic-collect-cli-'));
  ...
  const code = await runCli([
    'collect',
    '--root', root,
    '--probe', path.resolve('resources/probe.js'),
    '--host', '127.0.0.1',
    '--port', '0',
  ], io);
