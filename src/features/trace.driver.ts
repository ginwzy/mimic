...
function config(value: JsonValue | undefined): TraceConfig['op'] {
...
export const traceDriver: Driver = {
  reduceReports: (reports) => ({
    dynamicCode: reports.flatMap((report) => (report as Data).dynamicCode as JsonValue[]),
  }),
  open: (port) => {
