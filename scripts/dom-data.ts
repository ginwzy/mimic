...
const OUTPUT = path.join(ROOT, 'src/features/dom.data.ts');
const MISSING_OUTPUT = path.join(ROOT, 'src/features/dom.missing.data.ts');
const BASELINES = [
  { id: 'wv138', file: 'resources/baselines/android-webview-v138.json' },
  { id: 'c143', file: 'resources/baselines/linux-chrome-v143.json' },
  { id: 'c148', file: 'resources/baselines/macos-chrome-v148.json' },
  { id: 'c149', file: 'resources/baselines/macos-chrome-v149.json' },
] as const;
...
function fail(message: string): never {
  throw new Error(`dom-data: ${message}`);
}
...
function render(protos: readonly ProtoData[]): string {
  ...
  return `// Generated by scripts/dom-data.ts. Do not edit.\n\n`
...
async function main(): Promise<void> {
  ...
  if (args.length > 1 || (args.length === 1 && args[0] !== '--check')) fail(`usage: node scripts/dom-data.ts [--check]`);
  ...
  if (args[0] === '--check') {
    for (const file of files) {
      ...
      if (current !== file.content) fail(`${path.relative(ROOT, file.path)} is stale; run node scripts/dom-data.ts`);
    }
    return;
  }
