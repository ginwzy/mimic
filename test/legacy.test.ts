...
import { LegacyProfiles, MimicError } from '../src/index.js';
...
async function withProfiles(files: Record<string, unknown>, run: (profiles: LegacyProfiles) => Promise<void>): Promise<void> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'mimic-'));
  try {
...
test('LegacyProfiles wraps malformed JSON in the stable profile error contract', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'mimic-bad-json-'));
  try {
...
test('LegacyProfiles validates checksum-matched Shape artifacts through the strict codec', async () => {
  const profilesRoot = await mkdtemp(path.join(os.tmpdir(), 'mimic-profile-artifact-'));
  const shapesRoot = await mkdtemp(path.join(os.tmpdir(), 'mimic-shape-artifact-'));
  try {
