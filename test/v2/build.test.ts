...
import { REQUESTED_JSDOM_VERSION } from '../../src/v2/node/metadata.js';

const root = process.cwd();
const production = path.join(root, 'build/v2');
const tests = path.join(root, 'build/v2-test');
...
function build(mode: 'production' | 'test', output?: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, ['scripts/build-v2.js', mode], {
      cwd: root,
...
test('production and test builds stay isolated', async () => {
  ...
  assert.ok(productionFiles.includes('src/v2/index.js'));
  assert.notEqual((await fs.stat(path.join(production, 'src/v2/cli.js'))).mode & 0o111, 0);
  ...
  assert.ok(testFiles.includes('test/v2/build.test.js'));
  assert.ok(testFiles.includes('src/v2/index.js'));
...
test('production build removes stale target files before compiling', async (t) => {
  const isolated = await fs.mkdtemp(path.join(os.tmpdir(), 'mimic-v2-build-'));
