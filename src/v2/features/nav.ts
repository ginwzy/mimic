...
import type { Connection, Shape, Support } from '../core/types.js';
...
const DEFAULT_CONNECTION: Connection = { effectiveType: '4g', downlink: 10, rtt: 50, saveData: false };
...
export const navFeature: Feature = {
  ...
  build: ({ profile, page, shape }) => {
    const connection = page?.connection || DEFAULT_CONNECTION;
    const connectionSupport: Support = page?.connection ? 'captured' : 'emulated';
    return {
      binds: [
