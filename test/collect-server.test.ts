...
async function temporaryRoot(t: test.TestContext): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'mimic-collect-server-'));
...
async function fixture(): Promise<{ profileRaw: Record<string, unknown>; probeSnapshot: Record<string, unknown> }> {
  ...
  const probeSnapshot = JSON.parse(
    await readFile(path.resolve('resources/baselines/android-webview-v138.json'), 'utf8'),
  ) as { meta: Record<string, unknown> } & Record<string, unknown>;
  delete probeSnapshot.meta.profile;
  return { profileRaw, probeSnapshot };
}

test('collect server serves the compiled collectors and one legacy POST creates immutable artifacts', async (t) => {
  ...
  try {
    ...
    const compiledIdentity = await readFile(new URL('../src/collect/browser.js', import.meta.url), 'utf8');
