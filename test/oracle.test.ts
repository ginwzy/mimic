...
for (const pair of pairs) {
  test(`runtime stays within the P0 oracle budget for ${pair.profile} x ${pair.baseline}`, async () => {
    const baseline = JSON.parse(readFileSync(path.resolve(`resources/baselines/${pair.baseline}.json`), 'utf8')) as ProbeSnapshot;
