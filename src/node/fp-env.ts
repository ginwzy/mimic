...
function profileId(raw: Data, recordId: string): { id: string; target: Target } {
  ...
  const target = identityTarget({ navigator: flattened, screen: object(raw, 'screen', 'screen') });
...
function toIdentityInput(id: string, raw: Data, target: Target): { data: Data; derived: string[] } {
  const navigator = object(raw, 'navigator', 'navigator');
...
export async function normalizeFpEnv(
  ...
  const shape = await targetShape(identified.target);
  const identity = toIdentityInput(identified.id, raw, identified.target);
  return normalizeIdentity(
    identified.id,
    identity.data,
    { source, shape, derived: identity.derived },
  );
}
