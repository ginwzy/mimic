...
import type { Driver, DriverSession, Port } from '../engine/types.js';
...
function createDomSession(): DriverSession {
  // Detached iframe fallback belongs to this task, never another live or disposed Realm tree.
  let sharedCanvasFactory: (() => object) | null = null;
  const open: DriverSession['open'] = (port) => {
  ...
  return { open, close: () => { sharedCanvasFactory = null; } };
}

export const domDriver: Driver = {
  createSession: createDomSession,
  open: (port) => createDomSession().open(port),
};
