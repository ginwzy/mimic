...
本指南覆盖稳定 SDK、命令行、执行 HTTP API 与真机采集服务。默认 `mimic` 包入口和 `mimic` 命令均指向
v2;旧实现只通过 `mimic/legacy` 暂时提供。
...
`mimic/legacy` 只提供短期兼容,默认 import 和 CLI 已使用 v2;应尽快迁移到 `createMimic`。
